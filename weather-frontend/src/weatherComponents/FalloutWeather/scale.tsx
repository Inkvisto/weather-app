...
export const Scale= ({scaleType,scaleSide}:ScaleProps) => {
   ...
   const pressureFormula = (pressure:number) => {
      ...
      if(scale.current !== null){
         scale.current.style.height = `${height}px`
   ...
   React.useEffect(()=>{
      ...
      if(typeof data.main === 'object'){
         if(scaleType === 'pressure'){
            ...
            const height = function getRndInteger(min:number, max:number) {
               return Math.floor(Math.random() * (max - min) ) + min;
           }
            if(scale.current !== null){
            scale.current.style.height = `${ height(30,100)}px`
            }
         }
      }
   },[data])
 ...
 <div className={styles[sideStyles]}>
    <div className={styles.scale}><div ref={scale} className={styles.inputOfScale}></div></div>
            </div>
      </div>
   )
